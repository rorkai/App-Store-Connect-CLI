...
Set `ASC_WEB_APPLE_ID` to choose the Apple Account for the `asc web` commands
that resolve or create a session, including `asc web auth login`, without
repeating `--apple-id`; an explicit flag still wins. The cache-management
commands above (`status`, `export`, `import`, `logout`) ignore it. See
...
A runner that cannot answer prompts can create the session non-interactively
instead: `ASC_WEB_PASSWORD` supplies the Apple Account password and
`ASC_WEB_2FA_CODE_COMMAND` names a shell command that prints the two-factor
code. Both are read only when a fresh login is required, and a password taken
from the environment is never written to the saved-password store. Set
`ASC_WEB_DONT_STORE_PASSWORD=1` to turn off saved-password reads and writes
altogether. See
...
<CodeGroup>
  ...
  # --profile is accepted on either side of the command.
  asc --profile "WorkApp" apps list
...
* Has correct permissions: `chmod 600 AuthKey_*.p8`, or re-run `asc auth login --fix-permissions` to let asc change an ordinary over-permissive file to `0600` (owner-unreadable files on macOS require a manual `chmod`)
