...
Flags covered today: `--build-id`, `--app` (including `ASC_APP_ID`),
`--version-id`, `--submission-id`, `--info-id`, `--localization-id`,
`--subscription-id`, `--iap-id`, the `--id`/`--group` flags on `testflight groups`,
`--id` on `apps`, `subscriptions`, `iap`, and `bundle-ids`, and `--bundle` on
`bundle-ids capabilities`. Other ID flags still take bare IDs.
