...
| `6`  | Read-only refusal      | `ASC_READ_ONLY` or `--read-only` blocked a mutating request     |
...
| 401 Unauthorized  | `3`       | Authentication failure (special case)  |
...
An unknown flag is a usage error: exit code `2`, nothing on stdout, and a
diagnostic on stderr that names up to three flags the command actually defines
under `Try:`. Suggestions come from the command's own flag set, so a suggested
flag is always accepted by the command that printed it:

```
$ asc submit status --app <APP_ID>
Error: unknown flag `--app` for `asc submit status`
Try:
  --id
For help:
...
The flag value is never echoed, and a flag removed in 5.0 names its
replacement instead (see [Migrate to 5.0](/migrate-to-5-0)).
...
<Note>
  Missing, invalid, unauthorized, and forbidden credentials all return exit code `3`, so scripts can handle authentication failures consistently.
</Note>
