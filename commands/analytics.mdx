...
When viewing all reports without `--instance-id`, report-instance requests use
bounded parallelism while preserving report order. If one request fails, the
remaining sibling requests are canceled and the original failure is returned.
