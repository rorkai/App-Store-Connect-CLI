...
<Note>
  Apple Pay certificates are created against a merchant ID. Pass `--merchant-id` with
  these four types, either through `asc certificates create` or
  `asc merchant-ids certificates create`. Without `--merchant-id` the command exits with
  code 2 before generating a CSR, and `--merchant-id` is rejected for every other
  certificate type.
</Note>

```bash  theme={null}
asc certificates create --certificate-type APPLE_PAY_MERCHANT_IDENTITY \
  --merchant-id "MERCHANT_ID" \
  --csr ./merchant.csr

asc merchant-ids certificates create --merchant-id "MERCHANT_ID" \
  --certificate-type APPLE_PAY_RSA \
  --generate-csr \
  --key-out ./signing/merchant.key \
  --csr-out ./signing/merchant.csr
```
