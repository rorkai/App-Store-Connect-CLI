...
Get the app related to an Xcode Cloud product, with optional sparse fields
on included resources. The legacy AppInfo selector below is forwarded with
a deprecation warning because API 4.5 removed `AppInfo.kidsAgeBand`:
...
* `--app-info-fields` - Legacy sparse fields for included app info: `kidsAgeBand`.
  API 4.5 removed this property; the CLI still forwards it with a stderr
  deprecation warning and automatically includes `appInfos`. Use
