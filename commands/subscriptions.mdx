...
Manage legacy v1 group-scoped display names for different locales.
...
<Note>
  `asc subscriptions groups localizations ...` remains the stable v1,
  group-scoped workflow. API 4.4.1 version-scoped localizations use
  `asc subscriptions groups versions localizations ...` and the v2 API. The
  existing commands are not redirected or removed, so choose the subtree that
  matches whether the localization belongs to a group or a group version.
</Note>
...
existing `asc subscriptions localizations` and `asc subscriptions images`
commands remain product-scoped v1 workflows; use the commands below for the
new version-scoped resources.
...
* `list`: `--state`, `--fields`, `--include`, `--subscription-fields`,
  `--image-fields`, `--localization-fields`, `--image-limit`,
  `--localization-limit`, `--limit`, `--next`, and `--paginate`
* `view`: `--fields`, `--include`, `--subscription-fields`, `--image-fields`,
  `--localization-fields`, `--image-limit`, and `--localization-limit`
