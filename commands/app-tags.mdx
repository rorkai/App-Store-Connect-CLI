...
use `view` or `update` against one specific tag.
...
<ParamField path="--include" type="string">
  Include related resources. The `territories` value is deprecated in API 4.5
  and emits a warning on stderr.
</ParamField>

<ParamField path="--territory-fields" type="string">
  Deprecated in API 4.5. Selects territory fields when `--include territories`
  is set; the supported field is `currency`. Emits a warning on stderr.
</ParamField>

<ParamField path="--territory-limit" type="integer">
  Deprecated in API 4.5. Limits included territories per tag when
  `--include territories` is set. Emits a warning on stderr.
</ParamField>
...
### 4. Legacy territory lookups (deprecated)

These compatibility examples use API 4.5-deprecated territory selectors and
endpoints. Each invocation emits one deprecation warning on stderr. Omit
territory selections and lookups in new workflows.
...
The legacy `territories` command returns full territory resources;
`territories-links` returns relationship IDs.
