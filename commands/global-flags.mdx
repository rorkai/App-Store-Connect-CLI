...
`--profile` is the one global flag accepted on either side of the command name:
both forms above select the same credential. It still requires a value wherever
it appears, and a value-less `--profile` exits with code `2`. Use the
`--profile=NAME` spelling when the profile name is also a subcommand name, and
note that the selector must come before any positional argument.
...
Global flags must appear before the top-level command, except `--profile`, which
is accepted anywhere the command's own flags are:
...
Any other global flag written after the command name is an unknown flag for
that command: the CLI exits with code `2` and names the placement rule.
...
Refuse every mutating request (`POST`, `PATCH`, `PUT`, `DELETE`) before it is sent, for this invocation only.

```bash  theme={null}
asc --read-only apps list
...
**Environment variable**: `ASC_READ_ONLY` (accepts `true/false`, `1/0`, `yes/no`, `y/n`, `on/off`)

Read-only mode is enforced at the transport layer of every Apple-facing client, so it covers commands that never had a `--confirm` flag. Reads, local-only commands, and credential minting keep working. Either source enables the mode; neither can disable the other. See [Read-only mode](/concepts/read-only-mode) for the full contract.
...
| `--read-only`   | `ASC_READ_ONLY`      | `true/false`, `1/0`, `yes/no`, `y/n`, `on/off` |
...
| `ASC_UPLOAD_TIMEOUT`         | Upload and download timeout     | `60s`, `5m`                       |
| `ASC_UPLOAD_TIMEOUT_SECONDS` | Upload/download timeout seconds | `300`                             |
