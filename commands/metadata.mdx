...
* `--if-exists skip` records an existing locale as `status: skipped` and leaves it untouched; `--if-exists update` applies the planned fields to it instead. `fail` keeps today's behavior and reports the conflict.
