...
When `--version` is omitted, the app's newest active editable App Store version
(`PREPARE_FOR_SUBMISSION`, `DEVELOPER_REJECTED`, `REJECTED`,
`METADATA_REJECTED`, `READY_FOR_REVIEW`, `WAITING_FOR_REVIEW`, or
`INVALID_BINARY`) is used, then a `DEVELOPER_REMOVED_FROM_SALE` version, and
finally the live version. The selected version is reported on stderr
and its version string names the `version/<version>/` directory. Pass
`--platform` when the app has candidate versions on more than one platform.
Resolution completes before any metadata file is written, so an ambiguous or
missing default leaves the output directory unchanged.

**Flags:**

* `--app` - App Store Connect app ID (or `ASC_APP_ID`)
* `--version` - App version string (e.g., `1.2.3`); defaults to the active editable version, then a developer-removed version, else the live version
* `--platform` - Optional platform: `IOS`, `MAC_OS`, `TV_OS`, or `VISION_OS`; also selects the default version
...
* `--confirm` - Confirm destructive operations (required for deletes, field clears, or `--review-dir` applies)
* `--if-exists` - Behavior when a locale already exists: `fail` (default), `skip`, `update`
* `--output` - Output format: `json`, `table`, `markdown`
* `--pretty` - Pretty-print JSON output
...
* A missing `app-info/` or `version/<version>/` directory leaves that scope unmanaged: its remote localizations are not read, planned, or deleted. An existing directory, even an empty one, manages every locale in that scope. A push still requires at least one metadata `.json` file somewhere under `--dir`, so an otherwise empty tree is rejected instead of deleting a whole scope
* Omitted fields are treated as no-op; they do not imply deletion
* Explicit `null` clears only supported nullable fields and requires an existing locale when it is the file's only intent
* `--if-exists skip` records an existing locale as `status: skipped` and leaves it untouched; `--if-exists update` applies the planned fields, including explicit `null` clears, to it instead and requires `--confirm` when a locale planned as a create clears a field. `fail` keeps today's behavior and reports the conflict.
...
To clear a supported nullable field, set it to JSON `null` in its localization
file. `subtitle`, `privacyPolicyUrl`, and `promotionalText` support explicit
clears. Omitted keys leave remote values unchanged; empty strings are not clear
instructions, and `null` is rejected for fields without clear support.
An explicit-locale file can combine a clear with other field updates.
Applying a planned clear requires `--confirm`; dry-run plans do not.

A clear-only file must name a locale that already exists in App Store Connect.
`metadata plan`, `metadata push`, and `metadata apply` report an error for a
missing locale instead of silently treating the clear as a successful no-op.
`metadata validate` accepts these explicit-locale clear-only files offline;
the remote locale existence check happens during plan, push, or apply.
...
It cannot contain `null` clear fields because its values fan out to multiple
locales; put a clear in the corresponding locale-specific file instead.
...
rm metadata/version/1.2.3/ja-JP.json
