...
cannot become telemetry dimensions. Schema v5 adds `attempted_child` only for
`unknown_child` events: the lowercased token is recorded when it is one of
`list`, `get`, `view`, `show`, `create`, `add`, `update`, `edit`, `set`,
`delete`, `remove`, `rm`, `download`, `upload`, `push`, `pull`, `sync`,
`fetch`, `status`, `info`, `run`, `start`, `stop`, `install`, or `help`.
Every other token, including IDs, paths, and flags, is recorded as `other`.
Leaf and group events set `attempted_child` to `null`. `session_id` is reused only within one CLI
...
low-cardinality allowlist, such as `required_input_missing`,
`file_invalid_format`, or `dsym_not_ready`.
