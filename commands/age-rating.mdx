...
The classification number must be nonempty. Omit both GRAC flags to leave it
unchanged. Use `--clear-grac-rating-classification-number` to remove it with an
explicit JSON `null`; the set and clear flags cannot be combined.

```bash
asc age-rating edit --id "DECLARATION_ID" --clear-grac-rating-classification-number
```
