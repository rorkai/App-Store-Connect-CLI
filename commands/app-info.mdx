...
<ParamField path="--fields" type="string">
  ...
  from `AppInfo`. The selector is still forwarded for compatibility and emits
  one deprecation warning on stderr. Use `asc age-rating view --app-info-id ID`
  for current age-rating data.
</ParamField>
...
<Warning>
  API 4.5 removed `AppInfo.kidsAgeBand`. Existing AppInfo selectors remain
  available with a stderr deprecation warning; Apple may reject the request.
  Migrate to `asc age-rating view --app-info-id "APP_INFO_ID"`.
  `AgeRatingDeclaration.kidsAgeBand` and the `--kids-age-band` flag on
  `asc age-rating edit` remain supported. The warning also covers AppInfo selectors on apps,
  localizations, Xcode Cloud product apps, and continuation URLs.
</Warning>
