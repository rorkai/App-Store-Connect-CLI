...
<ParamField path="--if-exists" type="string" default="fail">
  `fail`, `skip`, or `update` when Apple returns 409
  `ENTITY_ERROR.ATTRIBUTE.INVALID.DUPLICATE` because the locale already exists.
  `fail` is unchanged. `skip` reads the localization back and leaves it.
  `update` sends the same field write again. Other 409s still fail.
</ParamField>
