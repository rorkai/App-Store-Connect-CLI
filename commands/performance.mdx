...
## Performance Overview (API 4.5)

Read the aggregated overview used by Xcode performance reports:

```bash
asc performance overview --app "APP_ID" --output json
asc performance overview --app "APP_ID" --device-type "iPhone15,2" --output json --pretty
```

`--app` can also come from `ASC_APP_ID`. The optional `--device-type` filter
accepts comma-separated values. This endpoint has no pagination.

JSON output preserves the complete Apple payload: app metadata, regressions,
trends, metric datasets, and diagnostic signatures. Table and Markdown output
summarize the app, metric-category count, and insight counts. The command
negotiates Apple's `application/vnd.apple.xcode-overview+json` media type;
`asc api` directs callers here.
...
asc performance metrics view --build-id "abc-123-def"
...
Tracks storage footprint over time:

* Data usage growth
* Binary size regressions
* On-device storage impact
...
# Get current metrics
asc performance metrics view \
  --build-id "$CURRENT_BUILD" \
  --metric-type "MEMORY,LAUNCH" \
  > current_metrics.json

# Get previous metrics
asc performance metrics view \
  --build-id "$PREVIOUS_BUILD" \
  --metric-type "MEMORY,LAUNCH" \
