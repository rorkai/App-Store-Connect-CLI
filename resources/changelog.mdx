...
* `asc validate` is now the canonical App Store submission readiness report with broader checks and clearer upgrade guidance from the legacy `submit preflight` wrapper
...
* `feat(validate)`: Make `asc validate` the canonical App Store submission readiness command and deprecate the legacy `submit preflight` wrapper
