...
func artifactIPAInfoRows(result *ArtifactIPAInfo) ([]string, [][]string) {
	headers := []string{"Bundle ID", "Version", "Build", "Status", "Signature"}
	if result == nil {
		return headers, nil
	}
	return headers, [][]string{{result.BundleID, result.Version, result.BuildNumber, result.Status, result.SignatureVerification}}
}

func artifactPKGInfoRows(result *ArtifactPKGInfo) ([]string, [][]string) {
	headers := []string{"Product ID", "Version", "Install Location", "Status", "Signature"}
	if result == nil {
		return headers, nil
	}
	return headers, [][]string{{result.ProductID, result.Version, result.InstallLocation, result.Status, result.SignatureVerification}}
}
