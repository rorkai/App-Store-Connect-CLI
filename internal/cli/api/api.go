...
Examples:
  ...
  asc api DELETE /v1/betaGroups/GROUP_ID --confirm`,
		...
		Exec: func(ctx context.Context, args []string) error {
			...
			request, err := parseRequest(positional, query, *body, *bodyFile, *paginate, *confirm, *allowUnknownPath)
			if err != nil {
				return err
			}
...
func parseRequest(positional []string, query []string, body, bodyFile string, paginate, confirm, allowUnknownPath bool) (rawRequest, error) {
	if len(positional) < 2 {
	...
	hasBody := strings.TrimSpace(body) != "" || strings.TrimSpace(bodyFile) != ""
